...
clearBtn.addEventListener('click', function () {
  ...
  localStorage.setItem('tempStorageObject', ''); //or localStorage.clear();
...
function loadFromLocalStorage() {
  ...
  if (storedData) {
    ...
    tempStorageObject.images.forEach((text) => {
      const textSpan = document.createElement('span');
      textSpan.textContent = text.text; //text.text = first text is the array, 2nd text is the first property of the array, near line 132
      textSpan.style.left = text.left;
      textSpan.style.top = text.top;
      textSpan.classList.add('draggable');
      moodBoardEl.appendChild(textSpan);
    });
  }
}
